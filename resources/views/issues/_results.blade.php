<div class="mb-4 flex items-baseline justify-between gap-4">
    ...
    <span class="text-xs text-stone-500" data-issue-count>{{ trans_choice(':count issue|:count issues', $issues->count(), ['count' => $issues->count()]) }}</span>
</div>

@if ($issues->isEmpty())
    <x-empty-state
        :title="__('No issues found')"
        :description="! empty($filters) ? __('Try different filters or search terms.') : __('Create an issue from a project page.')"
    >
        @if (empty($filters))
            <x-slot name="action">
            ...
            <li class="list-row">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div class="flex shrink-0 gap-3 text-sm">
                        <button
                            type="button"
                            data-open-modal="{{ $issue->editModalName() }}"
                            class="text-stone-500 hover:text-stone-900"
                        >{{ __('Edit') }}</button>
                        <button
                            type="button"
                            data-open-modal="{{ $issue->deleteModalName() }}"
